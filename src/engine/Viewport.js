...
export class Viewport {
  constructor(canvas, options = {}) {
    ...
    // TransformControls
    this.selectionBox = new THREE.Box3Helper(this._selectionBounds, 0xffd166);
    ...
    this.transformControls.addEventListener('change', () => {
      ...
      if (selected) {
        ...
        const nextPosition = [position.x, position.y, position.z];
        this.editor.updateMachinePosition(machineId, nextPosition);
        this.onGizmoChange(machineId, nextPosition);
      }
    });
  ...
  _setSelection(machineId) {
    this.editor.selectMachine(machineId);
  }

  _syncNodes() {
    const machines = this.editor.getMachines();
    const currentIds = new Set(this.machineRoots.keys());

    for (const machine of machines) {
      const nodeId = machine.id;
      if (!this.machineRoots.has(nodeId)) {
        const node = this._createMachineNode(machine);
        this.machineRoots.set(nodeId, node);
        this.scene.add(node);
      } else {
        const node = this.machineRoots.get(nodeId);
        node.position.set(...machine.position);
        node.userData.baseColor = machine.color;
        node.userData.bodyMaterial.color.set(machine.color);
      }
    ...
    for (const nodeId of currentIds) {
      ...
      this.machineRoots.delete(nodeId);
    }
  ...
  _applySelectionState() {
    for (const [machineId, node] of this.machineRoots.entries()) {
      const isSelected = machineId === this.selectedMachineId;
      node.userData.bodyMaterial.emissive.setHex(isSelected ? 0x1b1f2c : 0x000000);
    }
  }

  _updateSelectionBox() {
    const selected = this.machineRoots.get(this.selectedMachineId);
    if (!selected) {
      this.selectionBox.visible = false;
      return;
    }

    this._selectionBounds.setFromObject(selected);
    this.selectionBox.visible = true;
    this.selectionBox.box.copy(this._selectionBounds);
  ...
  addMachine(machine) {
    this.editor.addMachine(machine);
  }
  ...
  removeMachine(machineId) {
    this.editor.removeMachine(machineId);
  }

  replaceMachines(machines) {
    this.editor.replaceMachines(machines);
  }

  clearMachines() {
    this.editor.clearMachines();
  }
